...
### 1. 最快启动方式（Docker 开发环境）

```bash
# 首次启动会自动：
# - 开启 ENABLE_IDENTITY=true
# - 补齐 Postgres / Redis 本地默认配置
# - 生成 config/identity.yaml（空 OIDC providers）
# - 跑 alembic migration
# - 生成 JWT 密钥
# - 初始化 bootstrap 管理员密码
...
- 密码：`DeerFlow123!`
- 重置 token：`deerflow-bootstrap-local`
...
```bash
make docker-start   # Docker 开发环境
# 或 make up        # Docker 生产编排
```

接下来：

1. Bootstrap 管理员用上面的本地默认账号（或你自定义的 `.env` 凭据）登录 `/login`。
...
如果你想在本地 Docker 环境里完整跑通“启动 -> 登录页 -> 管理员登录 -> 进入工作区”这条链路，推荐直接阅读：

- [本地 Docker 运行与登录指南](docs/LOCAL_DOCKER_LOGIN_GUIDE_zh.md)
...
`make docker-start` 在启动容器前会自动补齐本地缺失文件：
- `config.yaml`（从 `config.example.yaml` 拷贝）
- `extensions_config.json`（从 `extensions_config.example.json` 拷贝）
- `.env` 和 `frontend/.env`（从对应的 `.env.example` 拷贝）

Docker 开发环境默认把 `extensions_config.json` 里的 `filesystem` MCP 根目录设为容器内存在的 `/app`。如果你在页面里启用 filesystem MCP，却仍保留模板占位路径（例如 `/path/to/allowed/files`），前端会报 `Unable to add filesystem: <illegal path>`；把该路径改成容器内真实存在的目录即可。

如果未启用 identity（默认 `ENABLE_IDENTITY=false`），前端会直接进入工作区，不再错误跳转到 `/login`。只有显式开启 identity 并配置数据库、Redis、JWT/OIDC 后，登录页和 `/api/auth/*` 鉴权接口才会真正生效。

Docker 构建默认使用官方源；如果你的网络环境较受限，建议在启动前设置：
- `UV_INDEX_URL=https://pypi.tuna.tsinghua.edu.cn/simple`
- `NPM_REGISTRY=https://registry.npmmirror.com`
- `APT_MIRROR=mirrors.aliyun.com`（或你自己的 Debian 镜像域名）

后端 Docker 镜像现在对 Debian `apt-get` 增加了重试与退避逻辑，并保持换源为可选项，以降低构建过程中偶发 `502 Bad Gateway` 导致的失败率。
