# 我花三个月搭了一个AI Agent平台，结果被checkpoint.db坑惨了

> 一个从零开始设计、基于LangGraph的企业级AI超级智能体系统，遇到了所有私有化部署都会踩的坑。
> 这篇文章是对deer-flow-by-cc项目的完整复盘，从架构设计到生产落地，从多租户隔离到checkpoint爆盘，全程硬核无废话。

---

说到AI Agent平台，大家第一反应可能是Dify、FastGPT这类成熟产品。但如果你想要完全掌控——数据不出机房、模型自己选、身份系统自己管——那大概率会走上自研这条路。

这个项目就是这样一个产物：基于DeerFlow 2.0改造，加上了多租户隔离、权限体系、审计日志，把一个单用户研究助手，做成了企业级AI工作平台。

整个过程踩了不少坑，尤其是checkpoint那个——后面会详细说。

下面是完整的技术拆解，从架构到代码，从设计到生产，全程硬核。
...
**扩展：LangGraph 深度拆解**
22. [LangGraph 执行模型内部机制](#22-langgraph-执行模型内部机制)
23. [create_agent 内部实现](#23-create_agent-内部实现)
24. [Checkpoint 磁盘膨胀——排查与修复](#24-checkpoint-磁盘膨胀排查与修复)
...
## 十九、Checkpoint能不能接MySQL？

### 19.1 MySQL 方案到底行不行？——重新评估

之前我推荐了 Postgres 而否定了 MySQL，这个判断值得重新考虑。你提出的"公司有成熟的 MySQL 运维"是合理的架构决策——引入额外 PG 实例的运维成本，可能比写一个 MySQL adapter 更高。
